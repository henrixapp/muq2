...
modFine = LotkaEquation(numStepsFine,1.0,.5,.5,0.5,0.5)
modCoarse = LotkaEquation(numStepsCoarse,1.0,.5,.5,0.5,0.5)


# Create an exponential operator to transform log-conductivity into conductivity

# Combine the two models into a graph
def create_graph(mod):
    graph = mm.WorkGraph()
...
graphCoarse = create_graph(modCoarse)
graphFine = create_graph(modFine)
postDensCoarse = graphCoarse.CreateModPiece("Posterior")
postDensFine = graphFine.CreateModPiece("Posterior")
problemCoarse = ms.SamplingProblem(postDensCoarse)
problemFine = ms.SamplingProblem(postDensFine)
...
#mioptions['MLMCMC.Subsampling_0'] = 50
#mioptions['MLMCMC.Subsampling_1'] = 5
...
mioptions['PrintLevel']=3
startPoint=[0.5]
mimcmc = ms.MIMCMC(mioptions, startPoint, [problemCoarse, problemFine])
mimcmc.Run([startPoint])
...
# TODO(henrik): show the trajectory of the samples in phase diagramm
print(sampMat)
